...
def train(sess, saver, train_graph, valid_graph, trainDataStream, devDataStream, options, best_path):
    f_valid=open('./quora/logs/valid.txt','w',encoding='utf-8')
    ...
    for epoch in range(options.max_epochs):
        ...
        print('Epoch %d: loss = %.4f (%.3f sec)' % (epoch, total_loss / num_batch, duration),file=f_valid)
        # evaluation
        start_time = time.time()
        ...
        print('Train in epoch %d  ,Accuracy: %.2f , Evaluation time: %.3f sec' % (epoch,acc,duration),file=f_valid)
        ...
        #     best_accuracy = acc
        saver.save(sess, best_path,global_step=epoch)
...
#             if not var.name.startswith("Model"): continue
        ...
        saver = tf.train.Saver(vars_,max_to_keep=30)
